...
class UpgradeManager {
  ...
  checkLicense() {
    return this.paymentsManager_
        ...
        .then(activePurchases => {
          this.paid_ = (activePurchases && activePurchases.length) || _TCAP_CONFIG.devPretendPro;
          return this.optionsManager_.setValue('paidPro', this.paid_);
        })
        .then(() => Promise.resolve(this.paid_));
  }

  checkSetGMikeLicenseCode(code) {
    const paymentsManager = new GMikePaymentsManager(this.optionsManager_, code, this.requiresPaid_, this.handleError_.bind(this));
  ...
  initializeForUpgrade () {
    this.paymentsManager_
        ...
        .then(availabilty => {
          if (!availabilty) {
            this.displayAlert_('FYI: The licensing server appears to be blocked. Please reach out to George: g@georgemike.com');
          }
        })
        .catch(err => this.handleError_(err, 'Upgrade error.'));

    Array
        .from(document.querySelectorAll('.btn-buy'))
        .forEach(el => el.addEventListener('click', this.handleBuyButtonPress_.bind(this)));
  ...
  handleError_(err, defaultMessage) {
    ...
    const errrorContext = {err, defaultMessage, ver: _TCAP_CONFIG.versionText};
    if (err && err.stack) {
    ...
    if (reportable) {
      errorElement
          ...
          .addEventListener('click', this.handleErrorReport_.bind(this, errrorContext));
    }

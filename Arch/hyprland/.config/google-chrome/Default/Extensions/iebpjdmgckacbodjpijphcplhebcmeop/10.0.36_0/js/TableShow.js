...
class TableShow {
  constructor (wrapper, actions, userConfig) {
    ...
    this.lockedDown_ = this.userConfig_.requiresPaid && !this.userConfig_.paidPro;
  ...
  display () {
    try {
      ...
      const inPageSelection = tableDefs.selectedElement || tableDefs.selectedTableWrapper;
      const specialCaseTable = airtablePage || googleSheetsAppPage || pdfPage || powerBiPage || xmlPage;
      ...
      if (specialCaseTable) {
        ...
        if (tableDefs.tables.length === 0) {
          this.wrapper_.appendChild(this.displayAlertWell(chrome.i18n.getMessage('noTablesFound')));
      ...
      Array
          .from(this.wrapper_.querySelectorAll('.btn-report-page'))
          .forEach(el => el.addEventListener('click', this.handlePageReport_.bind(this, currentUrl)));
    } catch (err) {
      writeBadError(err);
    }
  }
  ...
  displayPowerBi() {
    this.activityLogger_.logEvent('page-content-powerbi', {});
  ...
  displayXmlWarning() {
    this.activityLogger_.logEvent('page-content-xml', {});
  ...
  displayAirTable() {
    this.activityLogger_.logEvent('page-content-airtable', {});
  ...
  displayGoogleSheets() {
    this.activityLogger_.logEvent('page-content-gsheets', {});
  ...
  displayPdf() {
    this.activityLogger_.logEvent('page-content-pdf', {});
    ...
    div
      .querySelector('.btn-launch-pdf')
      .addEventListener('click', () => {
        const currentUrl = this.api_.getCurrentWindowLocation();
        this.browserEnv_.createTab({url: `/pdf-extract.html?url=${currentUrl}`});
      });
  ...
  displayNoTablesHelp() {
    this.activityLogger_.logEvent('page-content-no-tables');
    const currentUrl = this.api_.getCurrentWindowLocation();
  ...
  displayHeader (nTables) {
    ...
    const globalErrorWrapper = newElement('div', {className: '_tc_error_wrapper'});
    wrapper.appendChild(globalErrorWrapper);

    if (hasTablesPresent && Math.random() < .333) {
  ...
  displayFeatureTips_(wrapper) {
    ...
    `;
    fyiSelectionBanner.addEventListener('click', () => fyiSelectionBanner.remove());
  ...
  updateHeaderTitle () {
    ...
    const message = (nSelected > 0)
      ? chrome.i18n.getMessage('tablesFoundAndSelectedTitle', [nTables, nSelected])
      : chrome.i18n.getMessage('tablesFoundTitle', [nTables]);
    document.querySelector('._tc_title .main').innerText = message;

    document.querySelector('.selection-bar').classList.toggle('no-selection', nSelected === 0);
    Array.from(document.querySelectorAll('.selection-bar ._tc-right-actions input'))
        .forEach(button => {
  ...
  displaySelectionHeaderActionWrap () {
    ...
    selectionActionWrap.querySelector('.action_select_all').addEventListener('click', this.selectAllTables.bind(this));
    selectionActionWrap.querySelector('.action_select_none').addEventListener('click', this.unselectAllTables.bind(this));
    selectionActionWrap.querySelector('.action_copy').addEventListener('click', this.handleCopySelectedTables.bind(this));
    selectionActionWrap.querySelector('.action_goog').addEventListener('click', this.selectedToGoogle_.bind(this));
    selectionActionWrap.querySelector('.action_excel').addEventListener('click', this.selectedToExcel_.bind(this));
  ...
  displayHeaderActionLinkWrap() {
    ...
    const optionsAction = newElement('a', {
      ...
      id: 'options_action',
      click: (e) => {
        this.activityLogger_.logEvent('popup-action-goto-options', {});
    ...
    const supportAction = newElement('a', {
      ...
      id: 'support_action',
      click: (e) => {
        this.activityLogger_.logEvent('popup-action-goto-support', {});
    ...
    const favoritesAction = newElement('a', {
      ...
      id: 'favorites_action',
      click: (e) => {
        this.activityLogger_.logEvent('popup-action-goto-activity', {});
      ...
      this.actions_
          .forEach(action => {
            ...
            const click = (e) => {
              ...
              if (disabled) {
                this.activityLogger_.logEvent(`table-action-disabled-${key}`, {key});
                this.handleDisabledAction_(action);
              } else {
                this.activityLogger_.logEvent(`table-action-cb-${key}`, {key});
                this.cbs[key](tableWrapperLocal);
              }
              return _tcStopEventProp(e);
            };
            ...
            el.appendChild(newElement('img', {
              src: chrome.extension.getURL('/') + action.icon,
            }));

            if (action.tooltip) {
              el.addEventListener('mouseenter', this.displayTooltipText_.bind(this, action.tooltip));
  ...
  displayTableListMissingElement_() {
    ...
    `;
    li.querySelector('.btn')
        .addEventListener('click', this.handleTableNotFound_.bind(this));

    return li;
  }

  displayLoader () {
    this.wrapper_.appendChild(newElement('img', {
  ...
  refresh (userInitiated) {
    ...
    this.activityLogger_.logEvent(`popup-action-refresh (${sourceDescriptor})`);
    ...
    chrome.tabs.query({active: true, currentWindow: true}, tabs => {
      ...
      if (hasRuntimeError) {
        this.display();
        this.displayDismissableError_('There was an error communicating with the page. Try again?');
        return;
      }
  ...
  googTable (tableWrapper) {
    return this
        .copyTable(tableWrapper, OutputFormat.GOOG)
  ...
  setTableSelection (tableWrapper, select, batchSelect, selectionIndex) {
    ...
    const canToggle = listEls.length
    ...
    if (select === true && batchSelect && this.lastSelectionIndex_ !== null) {
      ...
      this.selectedTables[tableWrapper.displayIndex] = tableWrapper;
      if (canToggle) {
        listEls[tableWrapper.displayIndex].classList.add('selected');
        this.lastSelectionIndex_ = selectionIndex;
      }
    } else if (select === false) {
      this.selectedTables[tableWrapper.displayIndex] = false;
      if (canToggle) {
  ...
  toggleTableSelection_ (tableWrapper, batchSelect, index) {
    var selected = false;
    if (this.selectedTables.hasOwnProperty(tableWrapper.displayIndex)) {
      selected = !!this.selectedTables[tableWrapper.displayIndex];
    }
  ...
  unselectAllTables () {
    const {tables} = this.api_.getTableDefs();
    tables.forEach(tableWrapper => this.setTableSelection(tableWrapper, false));
    this.updateHeaderTitle();
  }

  selectAllTables () {
    const {tables} = this.api_.getTableDefs();
    tables.forEach(tableWrapper => this.setTableSelection(tableWrapper, true));
    this.updateHeaderTitle();
  }
  ...
  getSelectedTables () {
    return Object
        .values(this.selectedTables)
        .filter(tableWrapperMaybe => !!tableWrapperMaybe);
  }
  ...
  handleCopySelectedTables () {
    this.shardRollUpExtract_()
        .then(datasets => this.shardedResponsesToClipboard_(datasets, OutputFormat.CLIPBOARD))
  ...
  shardedResponsesToClipboard_(datasets, outputFormat) {
    ...
    const str = TableUtil.arrayOfArraysToString(arrayOfArrays, _TCAP_COPY_CONST.rowSeparator, _TCAP_COPY_CONST.colSeparator);
    const stringData = TableUtil.postProcessFinalString(str, outputFormat, _TCAP_COPY_CONST);
  ...
  selectedToGoogle_ () {
    this.shardRollUpExtract_()
        .then(datasets => this.shardedResponsesToClipboard_(datasets, OutputFormat.GOOG))
        .then(() => this.googDocCreate_())
        .catch(err => this.displayDismissableError_(err.message));
  }
    ...
    return Promise
        ...
        .then(responses => {
          ...
          if (allDatasets.length < tableWrapperCount) {
            throw new Error(`Expected ${tableWrapperCount} datasets, Received ${allDatasets.length}`);
          }
  ...
  handlePageReport_(currentPage) {
    const data = {
      ...
      licenseCode: this.userConfig_.licenseCode,
    };
  ...
  handleTableNotFound_() {
    this.activityLogger_.logEvent('popup-action-selection-workshop', {});
  ...
  openWorkshopForTable_(tableWrapper) {
    this.activityLogger_.logEvent('popup-action-selection-workshop-for-table', {});

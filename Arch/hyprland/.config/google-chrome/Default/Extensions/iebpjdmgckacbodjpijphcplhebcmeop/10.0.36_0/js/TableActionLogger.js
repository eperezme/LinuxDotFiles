...
class TableActionLogger {
  ...
  logPublishableTableAction(publishableTable, tcAction) {
    ...
    const {sourceUrl: url, pageTitle, tableDef, treatAsTable} = publishableTable;
  ...
  logWrapperAction(tableWrapper, tcAction) {
    ...
    return this.logActionForTableWrapperDefs_(tcAction, url, pageTitle, [def], tableWrapper.getReproOptions());
  }

  logWrapperDefAction(tableWrapperDefs, tcAction, url, pageTitle) {
    return this.logActionForTableWrapperDefs_(tcAction, url, pageTitle, tableWrapperDefs);
  }
  ...
  logActionForTableWrapperDefs_(tcAction, url, title, defs, reproOptions) {
    ...
    return this.logActionForTCTableDefs_(tcAction, url, title, tcTableDefs, reproOptions);
  }
  ...
  getLogDataForUrlAndDef(url, tableDef) {
    return this.getLogDataForUrl(url)
        .then(logData => {
          if (tableDef) {
            const { pathTo } = tableDef;
            const key = `dom://${pathTo}`;
...
const _tcLogTableWrapperMessageAction = (tableWrapper, messageAction, outputFormat=null, userConfig=null) => {
  const actionLogger = new TableActionLogger(userConfig);

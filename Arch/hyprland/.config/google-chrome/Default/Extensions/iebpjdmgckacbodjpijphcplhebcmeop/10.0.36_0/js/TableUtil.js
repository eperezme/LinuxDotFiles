...
function _tcCreateDocumentFromHTMLContent(htmlContent) {
...
function _tcGetPageyXPath(element) {
  ...
  const isButtony = ["button", "input", "a"].includes(tagName);
  if (!isButtony) {
    return null;
  }
  ...
  if (isPagey) {
    const pagingClassName = Array
        .from(element.classList)
        .filter(c => {
          const lc = c.toLowerCase();
          return lc.includes("next");
        })[0];
    return `//${tagName}[contains(@class,'${pagingClassName}')]`;
  }
...
function _tcGetSingleElementBySelector(selectorOrXpath, orMatchingInnerText=null) {
  ...
  try {
    ...
    if (selectorOrXpath.startsWith("/")) {
      return _tcGetElementByXpath(selectorOrXpath);
    }

    const matchingElements = Array.from(document.querySelectorAll(selectorOrXpath));
    if (matchingElements.length === 1) {
      return matchingElements[0];
    }
    if (orMatchingInnerText) {
      const matchingInnerTextElements = matchingElements.filter(e => e.innerText === orMatchingInnerText);
      if (matchingInnerTextElements.length === 1) {
        return matchingInnerTextElements[0];
      }
    }
  } catch (err) {}
  return null;
...
function _tcSpecificGetSelectorForEl(el) {
  ...
  const classes = Array.from(el.classList).filter(className => !className.startsWith('_tc_'));
  if (classes.length === 1) {
  ...
  const matchingElement = _tcGetSingleElementBySelector(selector);
  if (matchingElement) {
    return selector;
  }
...
function _tcGetPathTo(element) {
  ...
  const lcTagName = element && element.tagName && element.tagName.toLowerCase();
  if (lcTagName === 'html') {
    return '/html[1]';
  }
  ...
  for (let i = 0; i < siblings.length; i++) {
    ...
    if (sibling.nodeType === 1 && sibling.tagName.toLowerCase() === lcTagName) {
      ix++;
    }
  }
}
...
function _tcGetNextPageElement(currentPageNum) {
  ...
  return _tcGetElementByXpath(`//a[contains(@href,'page=${currentPageNum + 1}')]`);
}
...
function _tcWaitForPathOrSoloSelector(path, selector, maxWait) {
  ...
  const elViaSelector = _tcGetSingleElementBySelector(selector);
  if (elViaSelector) {
...
function _tcGetElementByXpath(path, doc) {
  ...
  while (path.length) {
    ...
    const el = doc.evaluate(exhaustivePath, doc, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
    if (el) {
      return el;
    }
  }
...
function _tcGetRepresentativeRow(dataArray) {
  ...
  let i = 0;
  while (firstUsefulRow && firstUsefulRow.length <= 1 && i + 1 < dataArray.length) {
    i++;
...
function isNodeWorkshopChild(node) {
  while (node) {
    if (node.tagName && node.tagName.toUpperCase() === 'TC-WORKSHOP') {
      return true;
    }
...
function _tcGetSelectedNodeFromSelection(frameWindow) {
  ...
  try {
    ...
    if (!startContainer || !startContainer.parentNode || !startContainer.parentNode.parentNode) {
      return null;
    }
...
function extractConfigToEnglish(extractConfig) {
  const english = [];
  extractConfig.getLinkUrls &&
      english.push('Extract link URLs');
  ...
  extractConfig.deleteEmptyRows &&
      english.push('Delete empty rows');

  if (extractConfig.ignoreImages) {
    english.push('Ignore images');
  } else if (extractConfig.extractImageSrc) {
  ...
  extractConfig.ignoreHiddenTables &&
      english.push('Ignore hidden tables');
...
function _tcGetBasePath() {
  if (!_tcBasePath || !_tcCurrentPath) {
    _tcCurrentPath = window.location.href.split('/').slice(0, -1).join('/') + '/';
...
const TableUtil = {
  postProcessFinalString: function(str, outputFormat, {rowSeparator, colSeparator}) {
    if (!str || !str.trim()) {
      return str;
    }

    if (outputFormat == OutputFormat.MARKDOWN) {
      const asArray = TableUtil.stringToArrayOfArrays(str, rowSeparator, colSeparator);
      return markdownTable(asArray);
    }

    if (outputFormat == OutputFormat.GOOG || outputFormat == OutputFormat.OFFICE365) {
  ...
  tableNodeToString: function(table, rowSeparator, colSeparator, userConfig, isRootTable) {
    if (!table) {
      return '';
    }

    const array = TableUtil.nodeToArrays(table, rowSeparator, colSeparator, userConfig, isRootTable);
  ...
  trNodeToRowArray : function(node, colSeparator, userConfig) {
    ...
    const str = _tcNodeToStringRoot(node, '', colSeparator, userConfig, false, opts);
    return str.split(colSeparator);
  },

  nodeToArrays : function(node, rowSeparator, colSeparator, userConfig, isRootTable) {
    const opts = {trimTrailingColSep: true};
    const str = _tcNodeToStringRoot(node, rowSeparator, colSeparator, userConfig, isRootTable, opts);
    const rows = str.split(rowSeparator);
...
function isSpaceNeeded(str, rowSeparator, colSeparator) {
  if (!str) {
    return false;
  }
  const lastCharacter = str[str.length - 1];
  const penultimateCharacter = (str.length > 1)
      ? str[str.length - 2]
      : null;

  if (lastCharacter === rowSeparator || lastCharacter === colSeparator
...
function _tcIsNodeWithinTable(node) {
  const tableTags = ['TD', 'TR', 'TBODY', 'TABLE'];
  while(node && node != document.body) {
    const ucTagName = node.tagName.toUpperCase();
    if (tableTags.indexOf(ucTagName) !== -1) {
      return true;
    }
...
// NOTE(gmike): Does not return leading/trailing spaces.
function _tcImageToString (extractImageSrc, ignoreImages, child) {
  if (ignoreImages) {
    return '';
  }

  if (extractImageSrc && child.src) {
    let imageSrc = child.src;
    if (imageSrc.indexOf(_TCAP_CONFIG.extBase) !== -1) {
      imageSrc = imageSrc.replace(_TCAP_CONFIG.extBase, '');
    }

    if (imageSrc.indexOf(_TCAP_CONFIG.chromeExtBase) !== -1) {
  ...
  function determineUniques(aArray, bMap, aUniques) {
    aArray.forEach((el, i) => {
      if (!bMap.hasOwnProperty(el.path)) {
        ...
        const newList = bMap[el.path].filter(existing => el.index != existing.index);
...
function _tcPopulateRowspanVals(array, table, rowspans, {deleteEmptyRows}) {
  if (!rowspans || rowspans.length === 0 || !table || !array || array.length === 0) {
    return;
  }
...
function _tcIsElementPager(node) {
  ...
  const hasNextOrPrev = innerText.includes('Next page') || innerText.includes('Previous page');
  const isTinyElement = innerText.length < 30;

  return (className.includes('pagination')
      || className.includes('pager')
      || (hasNextOrPrev && isTinyElement));
}

function _tcArbNodeToArray(node, userConfig, isRoot, depth, path, allRows, currentRow) {
  ...
  if (ignoreHiddenPageElements && _tcIsElementHidden(node)) {
    return;
  }
  ...
  if (!ignoreImages && extractImageSrc && 'I' === ucNodeName && node.childNodes.length === 0 && !node.innerHTML) {
    const content = window.getComputedStyle(node, ':before').getPropertyValue('content');
  ...
  if (!ignoreImages && 'SPAN' === ucNodeName && node.getAttribute('role') === 'img' && node.getAttribute('aria-label')) {
  ...
  const soleChild = node.parentElement && node.parentElement.childNodes.length === 1;
  ...
  if (node.childNodes.length) {
    ...
    while (child) {
      ...
      } else if (ignoreHiddenPageElements && _tcIsElementHidden(child)) {
        // Ignore elements that are not visible.
      } else {
        const localPath = `${path}.${child.nodeName}`;

        // Don't care about depth if only 1 child.
        const newDepth = (child.childNodes.length === 1 && depth !== 2) ? depth : depth + 1;
        _tcArbNodeToArray(child, userConfig, false, newDepth, localPath, allRows, currentRow);
  ...
  } else if ('#TEXT' == ucNodeName && ((node.nodeValue && node.nodeValue.trim()) || soleChild)) {
    const val = TableUtil.nodeVal(node).trim()
    currentRow.push({val, path});

    if (depth === 2) {
      currentRow.push({type: 'cell-break', comment: `Break @ depth (${depth}) for text node.`});
    }
  ...
  if (getLinkUrls && 'A' == ucNodeName) {
    ...
    if (href) {
      ...
      if (url) {
        _tcInsertIntoCurrentRowBeforeCellBreak(currentRow, {val: `(${url})`, path});
      }
    }
  } else if ('INPUT' === ucNodeName) {
    currentRow.push({val: _tcInputToString(node), path});
  } else if ('IMG' === ucNodeName || ('SVG' === ucNodeName && ignoreImages)) {
    currentRow.push({val: _tcImageToString(extractImageSrc, ignoreImages, node), path});
  }

  if (depth === 2 && node.nodeType && node.nodeType === 1) {
    currentRow.push({type: 'cell-break', comment: `Break @ depth (${depth}) for element.`});
  }
}

function _tcNodeToStringRoot(node, rowSeparator, colSeparator, userConfig, isRootTable, postOpts) {
  let str = _tcNodeToString(node, rowSeparator, colSeparator, userConfig, isRootTable);
...
function _tcNodeToString(node, rowSeparator, colSeparator, userConfig, isRootTable) {
  let str = '';
  ...
  if (addNewLinesForParagraphs && ('PRE' === ucNodeName || 'CODE' === ucNodeName)) {
  ...
  if ('TABLE' === ucNodeName && isRootTable === false) {
    ...
    const subTableString = _tcNodeToString(node, rowSeparator, colSeparator, userConfig, true);
    return _tcTableStringToCellString(subTableString, rowSeparator, colSeparator);
  }
  ...
  if (node.childNodes.length) {
    ...
    while (child) {
      ...
      if (ignoreHiddenPageElements && _tcIsElementHidden(child)) {
        child = child.nextSibling;
        continue;
      }
      ...
      let nodeStringValue = _tcNodeToString(child, rowSeparator, colSeparator, userConfig, false);

      if ('TR' == ucTag) {
        if (deleteEmptyRows && isNodeValueEmptyRow(nodeStringValue, colSeparator)) {
          // No-op. Skip this row.
        } else {
        ...
        if (isSpaceNeeded(str, rowSeparator, colSeparator)) {
          str += ' ';
        }
        ...
        if (getLinkUrls && 'A' == ucTag) {
          ...
          str += _tcImageToString(extractImageSrc, ignoreImages, child);
        }
      }
    ...
    str += TableUtil.nodeVal(node).trim();
  }

  if (addNewLinesForParagraphs && 'P' === ucNodeName && node.nextElementSibling) {
    str += '|';
  }
...
function _tcIsElementHidden(el) {
  if (!el) {
    return true;
  }
...
function _tcInsertIntoCurrentRowBeforeCellBreak(row, entry) {
  if (!row || !entry) {
    throw new Error('_tcInsertIntoCurrentRowBeforeCellBreak() -> Function params not valid.');
  }

...
class GPTBridge {
  ...
  applyPromptToData(prompt, data, expectedFirstRow, promptDataGenerator = null, max = 1, startingIndex = 0) {
    if (!prompt.includes("$")) {
      return Promise.reject(new Error("Temporary limitation: Prompts must operate on a column."));
    }
    ...
    const requestOptionsBase = {
      ...
      'Authorization': `Bearer ${_TCAP_CONFIG.gptApiKey}`
      },
    };
      ...
      promptDataGenerator = (prompt, data, index) => {
        ...
        if (placeholders) {
          ...
          placeholders.forEach((placeholder) => {
            ...
            fullPrompt = fullPrompt.replace(placeholder, `the text: \`${data[index][column]}\``);
          });
          return fullPrompt;
        }
    ...
    return Promise
        .all("X".repeat(max).split("").map((_, i) => {
          ...
          console.log("Magic column prompt: ", fullPrompt);
          return fetch(_TC_GPT_OPEN_API_URL, requestOptions)
            ...
            .then(response => this.processTableResponse_(response, effectiveIndex, data));
        }));
  }

  processTableResponse_(response, index, data) {
    const tokensUsed = (response && response.usage)

...
async function extractTables(pages, nCols, startRange, endRange, stackOfRows, minWidthFilter) {
  let content = [];
...
class PdfExtract {
  ...
  bindToFileInput_() {
    ...
    this.dropzone_.addEventListener('dragover', this.handleDragOver_.bind(this), false);
    this.dropzone_.addEventListener('dragleave', this.handleDragLeave_.bind(this), false);
    this.dropzone_.addEventListener('drop', this.handleManualFileDrop_.bind(this), false);
  }

  bindToUrl_(url) {
    this.activityLogger_.logEvent('pdf-extraction-url-load');
  ...
  clearTextSearchResults_(input) {
    const resultWrapper = input.parentElement.querySelector('.text-search-results');
  ...
  updateDirectionButtonGroup_() {
    Array
        .from(document.querySelectorAll('.direction-actions .btn'))
        .forEach(btn => {
  ...
  renderDataPreview_() {
    document.querySelector('.feedback-wrapper').classList.add('hidden');
    document.querySelector('.feedback-wrapper').classList.remove('show-report-button');
    this.updateDirectionButtonGroup_();

    const { nCols, startRange, endRange } = this.getExtractionParams_();

    if (!startRange.text || !endRange.text) {
      return this.handleError_(new Error('Missing first and last cell values.'));
    }

    extractTables(this.extractor_.getPages(), nCols, startRange, endRange, true, this.minWidth_)
  ...
  beginExtract_() {
    ...
    extractTables(this.extractor_.getPages(), nCols, startRange, endRange, this.isStackOfRows_, this.minWidth_)
        .then(({rows, widthRange}) => {
          this.renderShitDataForm_(widthRange);
          this.activityLogger_.logEvent('pdf-extraction-final');
          chrome.extension
              .getBackgroundPage()
              .saveTableLocally(this.createPublicTableFromData_(rows, this.pdfName_));
        })
        .catch(err => this.handleError_(err));
  }
  ...
  handleError_(err, defaultMessage="") {
    ...
    if (err && err.name && err.name === "PasswordException") {
      const pass = window.prompt(`${err.message}: Please provide a password for this PDF.`);
      if (pass) {
  ...
  handleFeedbackPress_(badData, _event) {
    ...
    wrapper.innerHTML = '';
    if (badData) {
      this.activityLogger_.logEvent('pdf-extraction-bad-data');

      const message = "I'm sorry to hear that! Please report the PDF and George will attempt to improve the extractor."
      wrapper.appendChild(createAlertPane(message, 'danger', false));
      document.querySelector('.feedback-wrapper').classList.add('show-report-button');
    } else {
      this.activityLogger_.logEvent('pdf-extraction-good-data');
  ...
  handleWidthTry_() {
    try {
      ...
      const message = `Please input a column width between ${niceNum(this.widthRange_.min)} and ${niceNum(this.widthRange_.max)}.`;
  ...
  handleFileAddition_(fileList) {
    ...
    if (pdfs.length === 1) {
      this.activityLogger_.logEvent('pdf-extraction-file-drop');

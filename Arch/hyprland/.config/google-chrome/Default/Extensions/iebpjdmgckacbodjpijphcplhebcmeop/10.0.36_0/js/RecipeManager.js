      ...
      <div class="collapsed-header">
        <div>
          ...
          <div class="recipe-metadata">
            ...
            <a class="clip-summary hidden" href="/clips.html?id=recipe-${recipe.id}"></a>
          </div>
        </div>
        <div>
          <input type="button"
          ...
          <input
              type="button"
              class="btn ${recipe.autoClip ? 'btn-warning' : 'btn-success'} btn-toggle-clipping"
              value="${recipe.autoClip ? "Stop Auto-Clipping" : "Start Auto-Clipping"}" />
          <input
              type="button"
...
function onLoad(element) {${recipe.onLoadFn || defaultOnLoadFn}}
      ...
      <input type="hidden" value="${recipe.autoClip || false}" class="recipe-auto-clip" />
    ...
    form.querySelector('.btn-toggle-status')
        .addEventListener('click', this.handleRecipeToggle_.bind(this, form, recipe.id));
    form.querySelector('.btn-toggle-clipping')
        .addEventListener('click', this.handleRecipeClippingToggle_.bind(this, form, recipe.id));
  ...
  getFormValues_(form) {
    ...
    if (onLoadFn.includes('function onLoad(element) {') && onLoadFn.endsWith('}')) {
  ...
  handleRecipeClippingToggle_(form, recipeId) {
    this.activityLogger_.logEvent('RecipeMan.clippingToggle');
  ...
  updateRecipeClippingToggleButtons_(form, recipe) {
    ...
    toggleBtn.value = recipe.autoClip ? "Stop Auto-Clipping" : "Start Auto-Clipping";
  ...
  handleRecipeDownload_(form) {
    this.activityLogger_.logEvent('RecipeMan.download');
  ...
  handleRecipeTest_(form) {
    ...
    const {urlRegex, urlExample, selector, fn, rpfn, onLoadFn} = this.getFormValues_(form);

    _testSelector(selector)
        .catch(err => this.handleFormError_(form, err));
    _testEvalFunction('(element) =>', fn, "Element")
        .catch(err => this.handleFormError_(form, err));
    _testEvalFunction('(element) =>', rpfn, "Row Element")
        .catch(err => this.handleFormError_(form, err));
    _testEvalFunction('(element) =>', onLoadFn, "OnLoad")
        .catch(err => this.handleFormError_(form, err));
  ...
  handleRecipeDelete_(form, recipe) {
    this.activityLogger_.logEvent('RecipeMan.delete');
    form.remove();
  ...
  handleFormSuccess_(form, recipe=null) {
    if (recipe) {
      form.querySelector('.collapsed-header .ro-recipe-name').innerText = recipe.name || 'None';
    }
...
function _testSelector(selector) {
  try {
    ...
    return Promise.reject(new Error(`The provided selector is not valid: ${selector}`));
  }
  return Promise.resolve();
}
...
function _testEvalFunction(sig, functionBody, functionName) {
  try {
    ...
    return Promise.reject(new Error(`There's a javascript error in your function (${functionName}).`));
  }

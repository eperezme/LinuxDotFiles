...
function getCachedExtensionUserConfig(context="") {
  if (USER_CONFIG) {
    ...
    if (USER_CONFIG.paidPro && USER_CONFIG_CACHE_HIT_COUNT > LICENSE_CHECK_NUM) {
      USER_CONFIG_CACHE_HIT_COUNT = 0;
      backgroundCheckSetLicense(USER_CONFIG.licenseCode, context);
    }
...
function backgroundCheckSetLicense(licenseCode, context="") {
  const url = `${_TCAP_CONFIG.licenseUrl}/${licenseCode}?activate=0&licensecheck=1&checkcontext=${context}`;
  fetch(url)
    .then(response => {
      if (response.ok) {

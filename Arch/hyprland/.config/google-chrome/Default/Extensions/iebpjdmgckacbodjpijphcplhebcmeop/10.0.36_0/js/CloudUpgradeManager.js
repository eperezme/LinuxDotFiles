...
class CloudUpgradeManager {
  ...
  handleFlowStart_() {
    ...
    this.authWrapper_
        ...
        .then(cloudLicenseResponse => {
          const { hasActiveCloudLicenses } = this.api_.processCloudLicenseResponse(cloudLicenseResponse);
          return this
              .updateCloudActivation_(hasActiveCloudLicenses)
              .then(() => {
                if (hasActiveCloudLicenses) {
                  window.location.href = "/account.html";
                } else {
                  window.open(_TCAP_CONFIG.cloudLicensePurchaseUrl + `?email=${this.user_.email}&tk=${this.token_}`);
  ...
  setToken_(googleAuthToken) {
    this.token_ = googleAuthToken;
    this.browserEnv_.getSyncStorageApi()
        .setP({
          googleAuthToken,
        })
        .catch(err => this.handleError_(err));

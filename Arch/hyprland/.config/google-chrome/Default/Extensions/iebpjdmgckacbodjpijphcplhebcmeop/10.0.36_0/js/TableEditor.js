...
class TableEditor {
    ...
    const dropdown = `
      <div class="dropdown">
        ...
        <ul class="dropdown-menu">
          ...
          <li><a href="#" class="export-airtable">Export to Airtable</a></li>
          <li role="separator" class="divider"></li>
  ...
  selectRow_(rowIndex, event) {
    ...
    if (this.selectedRows_.includes(rowIndex)) {
      ...
      if (shiftKey) {
        ...
        for (let i = Math.min(lastIndex, rowIndex); i <= Math.max(lastIndex, rowIndex); i++) {
  ...
  selectCol_(colIndex, event) {
    ...
    if (this.selectedCols_.includes(colIndex)) {
      ...
      if (shiftKey) {
        ...
        for (let i = Math.min(lastIndex, colIndex); i <= Math.max(lastIndex, colIndex); i++) {
  ...
  updateButtons_() {
    const disableHeaderRowDelete = this.disableSelectionActions_ || !this.table_.tableDataArray.length;
    const disableRowDelete = this.disableSelectionActions_ || !this.selectedRows_.length;
    const disableColumnDelete = this.disableSelectionActions_ || !this.selectedCols_.length;

    document
        .querySelector('.btn-delete-first-row').disabled = disableHeaderRowDelete;
    ...
    document.querySelector(".btn-clear-selection").disabled = disableColumnDelete && disableRowDelete;
  ...
  handleAddAiColumn_() {
    ...
    wrapper.innerHTML = `
      <div class="advanced-wrapper-inner ai-wrapper">
        ...
        <div clas="expectation-wrapper editable-wrappr">
          <div class="form-group">
  ...
  updateFirstRowHeadersSwitchLabel_() {
    ...
    const el = document.querySelector(".advanced-wrapper .first-row-headers-wrapper span.switch-label");
    el.innerText = firstRowHeaders ? "First Row is Header Row" : "First Row is Data Row";
  }

  updateAiButtonState_(requestInProgress) {
    document
        .querySelector('.btn-ai-preview').disabled = requestInProgress;
    document
        .querySelector('.btn-ai-apply').disabled = requestInProgress || !this.aiExpectationSuccess_;
  }
  ...
  async applyAiToDataArray_(maxRows=null) {
    ...
    return this.gptBridge_
        .applyPromptToData(prompt, this.table_.tableDataArray, expectation, null, maxRows, startingIndex)
        .then((responses) => {
          if (!responses.length) {
          ...
          if (isPreview) {
            ...
            if (!values || !values.length) {
              return Promise.reject(new Error("No values in Magic Columns response."));
            }

            if (values[0] !== expectation) {
              this.workInProgressTableDataArray_[startingIndex][0] = `Expected: ${expectation}, Received: ${values[0]}`;
              this.aiExpectationSuccess_ = false;
              return;
            }
          }
          ...
          responses.forEach((response, responseIndex) => {
            ...
            this.workInProgressTableDataArray_[index][0] = values.length ? values[0] : '--';
          });
  ...
  async handleAIPreview_() {
    if (!this.aiColumn_) {
      this.workInProgressTableDataArray_ = JSON.parse(JSON.stringify(this.table_.tableDataArray));
...
`;
  ...
  handleCloudAction_(tcAction) {
    ...
    this.logTableAction_(tcAction)
        ...
        .then(pageElement => {
          ...
          throw new Error("Unable to save table to Table Capture Cloud: Missing page element information.");
        })
        .catch(err => this.handleError_(err));
  }
  ...
  handleAirtable_() {
    const tcAction = {outputFormat: OutputFormat.AIRTABLE, messageAction: MessageAction.AIRTABLE_EXPORT};
    this.logTableAction_(tcAction);

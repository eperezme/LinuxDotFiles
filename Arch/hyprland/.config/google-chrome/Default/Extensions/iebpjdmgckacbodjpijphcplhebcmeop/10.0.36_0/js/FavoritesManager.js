...
class FavoritesManager {
  ...
  renderNoActivityMode_ () {
    document.querySelector('.activity-wrapper').classList.add('no-data-recorded');
  }
  ...
  renderPageElement_ (logItem, pageEntry) {
    ...
    pageElement.innerHTML = `
      <div class="header">
        <div class="left-side">
          <span
              class="icon-toggle-btn fave-toggle-btn glyphicon ${starred ? 'glyphicon-star' : 'glyphicon-star-empty'}"
              title="${starred ? 'Remove from favorites' : 'Add to favorites'}"></span>
          <span
              class="icon-toggle-btn domain-toggle-btn glyphicon glyphicon-globe ${domainWide ? 'glyph-on' : ''}"
              title="${domainWide ? 'Only show on this page' : 'Make available for entire domain'}"></span>
  ...
  toggleDomainWide_(logItem, pageEntry) {
    this.activityLogger_.logEvent('FavoritesManager.toggleDomainWide_');

...
function handlePagingContinuation(req) {
  ...
  if (selectionWorkshop && selectionWorkshop.isPaging(continuationKey)) {
    // No-op. Already paging for this continuation event.
    return;
  }

  _tcLogEvent('MessageAction.PAGING_CONTINUATION', {count});
  ...
  selectionWorkshop = new SelectionWorkshop(tabId, userConfig, workshopNode, installDate);
  selectionWorkshop.setPagingData(
    data, count, continuationKey, treatAsTable, pathToElement, pathToPager, pagerSelector, autoPageTable, firstPageSize);
  selectionWorkshop.renderPagingMode(false);
}

function handleInlineToWorkshopEvent({tabId, tableWrapper, userConfig, installDate}) {
  _tcLogEvent('MessageAction.SELECTION_WORKSHOP_FROM_INLINE');
...
function handleRecipeWorkshopClipEvent({tabId, userConfig, recipe}) {
  _tcLogEvent('MessageAction.SELECTION_WORKSHOP_RECIPE_CLIP');
...
function handleRecipeWorkshopEvent({tabId, userConfig, recipe, installDate, onLoad}) {
  _tcLogEvent('MessageAction.SELECTION_WORKSHOP_RECIPE');
...
function handleSelectionWorkshopEvent(
  ...
  _tcLogEvent('MessageAction.SELECTION_WORKSHOP', {hasSelection});
  ...
  if ((hasSelection && selectionNode) || !hasSelection) {
    ...
    const framesWithSelection = Array
    ...
    if (framesWithSelection && framesWithSelection.length == 1) {
      ...
      selectionNode = _tcGetSelectedNodeFromShadowRootSelection(workshopNode.shadowRoot);
      if (selectionNode) {
        selectionWorkshop = new SelectionWorkshop(tabId, userConfig, selectionNode, installDate);
        selectionWorkshop.renderSelectionMode();
      }
    }
  } else {
    if (_TCAP_CONFIG.devDebug) {
      ...
      console.log(`content.js::handleSelectionWorkshopEvent() -> No-op (${noOpContext})`);
    }
  }
}
...
function performRepro({repro, userConfig}, cb) {
  ...
  const isTableWrapper = (el.tagName === "TABLE");
  ...
  wrapper
      ...
      .catch(err => _tcPageToast("There was an error performing this action.", "error"))
      .finally(() => cb({}));
}
...
function initTableMan() {
  ...
  document.addEventListener('contextmenu', e => {
    ...
    if (selectionWorkshop) {
      selectionWorkshop.setContextMenuElement(contextMenuEl);
    }
  });
  ...
  getExtensionUserConfig()
    .then(userConfig => {
      ...
      // Manage GSheet names
      const sheetData = userConfig.enableGDriveWrite
          ? _tcGetSheetData()
          : null;
...
function _tcPerformExcelDatasetBuild(datasets, filenameTemplate, singleSheetExcelExport, userConfig) {
  ...
  if (singleSheetExcelExport) {
    ...
    return ExcelUtil.exportArrayOfArraysToExcelP(data, sheetname, filename, opts);
  }

  return ExcelUtil.multiSheetExport(datasets, filename, opts)
}

function _tcExcelDatasets({datasets, filenameTemplate, singleSheetExcelExport, userConfig}, cb) {
  return _tcPerformExcelDatasetBuild(datasets, filenameTemplate, singleSheetExcelExport, userConfig)
      .then(() => cb({}))
...
function _tcCopySelectedTables(req, manager) {
  ...
  new BrowserEnv()
      .sendMessage(params)
      .catch(err => _tcContentSwallowError(err, 'content.js::_tcCopySelectedTables()'));
}
...
function handleExtensionRequest(req, sender, cb) {
  ...
  if (_TCAP_CONFIG.devDebug) {
    console.log(`content.js::handleExtensionRequest() - Action = ${action}, Tab: ${tabId}`);
  }

  if (!action) {
    _TCAP_CONFIG.devDebug &&
        console.log(`content.js::handleExtensionRequest() - Early Quit: No Action`);
  ...
  if (req.windowName && req.windowName !== windowName) {
    _TCAP_CONFIG.devDebug &&
        console.log(`content.js::handleExtensionRequest() - Early Quit: Window name (${req.windowName} vs. ${windowName})`);
  ...
  if (!manager) {
    _TCAP_CONFIG.devDebug &&
        console.log(`content.js::handleExtensionRequest() - Early Quit: No manager`);
    cb({});
    return false;
  }
  ...
  const specialPageAction = (action === MessageAction.DISPLAY_INLINE || action === MessageAction.SELECTION_WORKSHOP);
  if (specialPageAction) {
    if (isAirtablePage) {
      return _tcPageToast("Airtable exports require a purpose-built extension.", "airtable");
    }
    if (isPowerBiPage) {
      return _tcPageToast("Table Capture does not work with PowerBI.", "error");
    }
  }
  ...
  if (MessageAction.DISPLAY_INLINE === action) {
    _tcLogEvent(action, {});
    const detachedTableShow = new DetachedTableShow(req.userConfig, getTableManagerForWindow(), tabId);
  ...
  _TCAP_CONFIG.devDebug &&
      console.log('content.js:: Request fall-through', manager.getAllTableDefs(), req);
...
if (_tcIsPageAnAdNetwork()) {
  ...
  try {
    ...
    _tcLogEvent('TC-GlobalErrorCaught');
  }
}

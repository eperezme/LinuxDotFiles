...
class WorkshopFrame {
  ...
  addModal(id, modal) {
    ...
    const container = document.querySelector("tc-workshop ._tc-modal-container");
    container.innerHTML = '';
  ...
  handleModalError_(err, message) {
    const detailedMessage = err && err.message
        ? err.message
        : typeof err === "string"
          ? err
          : "Unknown error";
  ...
  handleTopDock_() {
    this.dockTop();
    Array.from(document.querySelectorAll('tc-workshop .hint--top-left'))
        .forEach(el => {
  ...
  handleBottomDock_() {
    ...
    Array.from(document.querySelectorAll('tc-workshop .hint--bottom-left'))
        .forEach(el => {
  ...
  getActions_() {
    const actions = [
      ...
      /*
        // NOTE(gmike: 2-11-2023): Removing this.
        {
          title: 'Glimpse',
          key: 'glimpse',

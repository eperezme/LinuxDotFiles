# ssh
# Autogenerated from man page /run/current-system/sw/share/man/man1/ssh.1.gz
...
complete -c ssh -s D -d 'Specifies a local ``dynamic\'\' application-level port forwarding'
complete -c ssh -s E -d 'Append debug logs to log_file instead of standard error'
...
complete -c ssh -s J -d 'Connect to the target host by first making an ssh connection to the jump host…'
...
complete -c ssh -s n -d 'The recommended way to start X11 programs at a remote site is with something …'
complete -c ssh -o 'cert.pub' -d 'to identity filenames'
complete -c ssh -s L
complete -c ssh -s l -d 'Specifies the user to log in as on the remote machine'
complete -c ssh -s M -d 'Places the ssh client into ``master\'\' mode for connection sharing.  Multiple'
...
complete -c ssh -s O -d 'Control an active connection multiplexing master process.  When the'
...
complete -c ssh -s w -d 'Requests tunnel device forwarding with the specified tun(4) devices between t…'
complete -c ssh -s X -d 'Enables X11 forwarding'
complete -c ssh -s Y -d 'option and the ForwardX11Trusted directive in ssh_config(5) for more informat…'
complete -c ssh -s x -d 'Disables X11 forwarding'
complete -c ssh -s y -d 'Send log information using the syslog(3) system module'
complete -c ssh -o KLport -d 'for local,'
complete -c ssh -o KRport -d 'for remote and'
complete -c ssh -o KDport -d 'for dynamic port-forwardings'
complete -c ssh -s h -d option

# scp
# Autogenerated from man page /run/current-system/sw/share/man/man1/scp.1.gz
...
complete -c scp -s D -d 'Connect directly to a local SFTP server program rather than a remote one via …'
complete -c scp -s F -d 'Specifies an alternative per-user configuration file for ssh'
...
complete -c scp -s o -d 'Can be used to pass options to ssh in the format used in ssh_config(5)'
complete -c scp -s R -d 'option is used'
complete -c scp -s P -d 'Specifies the port to connect to on the remote host'
complete -c scp -s p -d 'is already reserved for preserving the times and mode bits of the file'
...
complete -c scp -s X -d 'Specify an option that controls aspects of SFTP protocol behaviour'

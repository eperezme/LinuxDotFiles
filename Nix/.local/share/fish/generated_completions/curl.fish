# curl
# Autogenerated from man page /run/current-system/sw/share/man/man1/curl.1.gz
...
complete -c curl -l ipfs-gateway -d 'Specifies which gateway to use for IPFS and IPNS URLs'
...
complete -c curl -s X -l request -d 'Change the method to use when starting the transfer. RS'

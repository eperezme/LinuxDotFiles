# sftp
# Autogenerated from man page /run/current-system/sw/share/man/man1/sftp.1.gz
...
complete -c sftp -s b -d 'Batch mode reads a series of commands from an input batchfile instead of R st…'
...
complete -c sftp -s o -d 'Can be used to pass options to ssh in the format used in ssh_config(5)'
complete -c sftp -o rm -d 'Echo of the command may be suppressed by prefixing the command with a `@\' cha…'
complete -c sftp -o @ls
...
complete -c sftp -s X -d 'Specify an option that controls aspects of SFTP protocol behaviour'
complete -c sftp -s h -d 'flag is specified, then symlinks will not be followed'
